...
$row = $resource->fetch_assoc();
    ...
    {
        echo "<script>alert('INCORRECT LOGIN DETAILS')
        window.location.replace('http://localhost/MISSIVE/login.php');</script>";
      
    }
$resource->free();
$res = $con->query("SELECT name,dp,phone,email,date FROM `test1`.`info` where username='$username'");
$ro=$res->fetch_assoc();
$name=$ro['name'];
$_SESSION['name']=$name;
$y=explode(" ", $ro['name']);
...
$resource = $con->query("SELECT dp FROM `test1`.`info` where username='$us'");
if(!$resource)
{
   echo $con->error;
}
...
.snav a:hover
{
  ...
  <div class="collapse navbar-collapse" id="navbarText">
    ...
    <span class="navbar-text" style="position:fixed;top:4px;right:28px">
      <?php echo " <img src='$dp' alt='Avatar' class='center zoom avatar' style='width:40px; height: 40px;position:fixed;top:10px;right:150px'>";?>
      <div class="dropdown" style="float:right;">
        <div class="row">
      <div class="column" style="width:2%;">
      </div>
      <div class="column">
        <?php echo " <p style='position:fixed;left:1375px;padding-bottom:8px;top:17px;color:white;font-size:21px'>$us</p>";  ?>
      </div>
    </div>
  ...
  <!-- <div class="tnav" >
    ...
    <div class="snav" >
      <div class="w3-sidebar  w3-bar-block" style="width:15%;margin-top:0px;margin-left: -3px; background-color:#848487;color:black;">
  ...
  <a href="dp1.php" class="w3-bar-item w3-button"><i style="font-size:24px" class="fa">&#xf1b0;</i>Change DP</a>
    ...
    <div class="des">
      <?php
        ...
        $con=mysqli_connect("localhost","root","","test1");
         $result="select username,img,title,location,body,dp from post order by date desc  ";
        ...
        echo "<p>"."$username"."</p>";
            ...
            echo "<br>";
            echo '<tr>';
            echo "<tr><td>"."<img class=\"iim\"src=\"$img\" width=\"805px\" height=\"500px\"\>"."</td></tr>";
            echo "<br>";
            echo "<td>"."$body"."</td>";
            echo "<br>";
            echo "</tr>";
...
<a class="likes"><i  id="dislike" class="fa fa-thumbs-down fa-2x" ></i></a>';
        ...
        <input id="submit-btn" type="submit" class="msub" name="commentsend" value="send" style="background-color:#10c254"/></div>';
